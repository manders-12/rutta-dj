...
class RatingsBackButton(Button):
    def __init__(self, db):
        super().__init__(label="Back", style=discord.ButtonStyle.secondary, custom_id="back_ratings")
        self.db = db
...
class RecView(View):
    def __init__(self, db):
        ...
        for name in recommended_by:
            self.add_item(RecButton(name, db))
        self.add_item(RecBackButton(db))

class RecBackButton(Button):
    def __init__(self, db):
        super().__init__(label="Back", style=discord.ButtonStyle.secondary, custom_id="back_rec")
        self.db = db
...
class RecButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if results:
            ...
            await interaction.response.edit_message(embed = embed, view=ResultsTable(results))
        else:
...
class RatingButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if results:
            embed = build_embed_table(results)
            await interaction.response.edit_message(embed = embed, view=ResultsTable())
        else:
