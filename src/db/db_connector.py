...
class DBConnector:
    ...
    def insert_recommendation(self, message_id, author, title, link, genres, tag):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (message_id, title, author, link, str(genres[0]), str(genres[1]) if genres[1] else None, tag))
        conn.commit()
    ...
    def get_all_genres(self):
        ...
        cursor = conn.cursor()
        cursor.execute('''
            SELECT DISTINCT genre1 COLLATE NOCASE AS genre FROM recommendations WHERE genre1 IS NOT NULL
            UNION
            SELECT DISTINCT genre2 COLLATE NOCASE AS genre FROM recommendations WHERE genre2 IS NOT NULL
        ''')
