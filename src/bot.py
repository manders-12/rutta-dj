...
def create_rating_embed(title, author, link, rating, explanation):
    try:
        embed = discord.Embed(title=f'Rating for {title}', description=explanation)
        ...
        embed = discord.Embed(title='Error', description='Failed to create rating embed.')
        embed.set_thumbnail(url=client.user.avatar.url)
        embed.set_footer(text='Rutta DJ Bot')
    return embed
...
def create_recommendation_embed(title, author, link, genre, tag):
    try:
        embed = discord.Embed(title=f'Recommendation: {title}', description=f'Genre: {genre}\nTag: {tag}')
        embed.set_thumbnail(url=client.user.avatar.url)
        embed.add_field(name='Author', value=author, inline=True)
        embed.add_field(name='Link', value=link, inline=True)
        embed.set_footer(text='Rutta DJ Bot')
        ...
        embed = discord.Embed(title='Error', description='Failed to create recommendation embed.')
        embed.set_thumbnail(url=client.user.avatar.url)
        embed.set_footer(text='Rutta DJ Bot')
    return embed
    ...
    if message.channel.name == TRACK_LIST_CHANNEL:
        ...
        if str(message.author.global_name).lower() == CONTROLLING_USER:
            ...
            if message.embeds:
                ...
                if genre and tag and title and author:
                    try:
                        db.insert_recommendation(message.id, author, title, link, genre, tag)
                        logging.info(f'Recommendation inserted: {title} by {author} ({link}) with genre {genre} and tag {tag}')
                    except Exception as e:
                        embed = discord.Embed(title='Error', description='Failed to insert recommendation.')
                        logging.error(f'Error inserting recommendation: {e}')
                    try:
                        if not embed:
                            embed = create_recommendation_embed(title, author, link, genre, tag)
                    except Exception as e:
                        embed = discord.Embed(title='Error', description='Failed to create recommendation embed.')
                        logging.error(f'Error creating recommendation embed: {e}')
                ...
                logging.info(f'No embeds found in message: {message.content}. Waiting for edit.')
                return False  # Wait for edit to process the recommendation
        else: return False  # Skip messages not from Rutta
        ...
        if str(message.author.global_name).lower() == CONTROLLING_USER and message.reference:
            try:
                ...
                logging.warning(f'Could not fetch replied message: {message.reference.message_id}. Error: {e}') 
            if replied_message:
                ...
                if title and author and link and rating is not None:
                    try:
                        db.insert_rating(message.id, recommended_by, title, link, rating, explanation)
                        embed = create_rating_embed(title, author, link, rating, explanation)
                        logging.info(f'Rating inserted: {title} by {author} ({link}) with rating {rating} and explanation "{explanation}"')
                    except Exception as e:
                        embed = discord.Embed(title='Error', description='Failed to insert rating.')
...
async def process(ctx):
    ...
    try:
        ...
        for channel in client.get_all_channels():
            if channel.name in [TRACK_LIST_CHANNEL, MUSIC_REVIEW_CHANNEL] and isinstance(channel, discord.TextChannel):
        ...
        for channel in channels:
            ...
            async for message in channel.history(oldest_first=True):
                if await process_message(message):
                    ...
                    skipped_count += 1
            logging.info(f'Processed {processed_count} messages, skipped {skipped_count} messages.')
        ...
        logging.info(f'Historical processing complete: {processed_count} processed, {skipped_count} skipped.')
...
async def on_message_edit(before, after):
    ...
    if after.channel.name == TRACK_LIST_CHANNEL and len(before.embeds) == 0 and len(after.embeds) > 0 and str(after.author.global_name).lower() == CONTROLLING_USER:
        ...
        if genre and tag and title and author and link:
            try:
                db.insert_recommendation(after.id, author, title, link, genre, tag)
                embed = create_recommendation_embed(title, author, link, genre, tag)
                await after.channel.send(embed=embed)
                logging.info(f'Recommendation inserted on edit: {title} by {author} ({link}) with genre {genre} and tag {tag}')
            except Exception as e:
